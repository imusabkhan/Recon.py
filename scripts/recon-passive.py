...
def screens():
    with open(scope,'r') as file:
        for domain in file:
            ...
            aquatone_command = [
                'sh', '-c', f'cat {domain}/Recon/httpx.txt | aquatone -ports 80,443 -out {domain}/Recon/screens'
            ]

            print(f"Screenshots In Progress - {domain} ")
            
            aquatone_results = subprocess.run(aquatone_command, text=True, stdout=subprocess.PIPE)    
            print(aquatone_results.stdout)
